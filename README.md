...
Во время разработки доступ к API Европейского Центрального Банка
иногда был ограничен сетевыми условиями.

Поэтому в коде реализован fallback-механизм: если API временно
недоступен, используются тестовые данные. Это позволяет продолжать
разработку и тестирование DAG без остановки пайплайна.

При доступности API данные загружаются напрямую из ECB.
